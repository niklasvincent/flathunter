...
class URLNormalizer {
	...
	public function getService() {
		if ( preg_match('/(?P<domain>[a-z0-9][a-z0-9\-]{1,63}\.[a-z\.]{2,6})$/i', $this->getHost(), $results) ) {
    ...
    public function normalize() {
        ...
        $url = $this->host . $this->port . $this->user . $this->pass . $this->path . $this->query . $this->fragment;
		$url = trim($url, '/');
		return $url;
    }
